...
const columns = [
    ...
    { field: 'kelas_head_catatan', headerName: 'Catatan', width: 130 },
    { field: 'kelas_head_total', headerName: 'Jumlah Record', width: 130 },
    { field: 'user_fullname', headerName: 'Oleh', width: 150, renderCell: (params) => {
        return (
            <>{params.row.user_fullname} <br/>pada : {params.row.kelas_head_created_at}</>
        );
    }},
    {
        field: 'status_error', headerName: 'Action', width: 150, renderCell: (params) => {
            return (
                <Link href={"/kelas/history/"+params.row.kelas_head_id}>
                  <Button style={{
                    marginTop: '10px',
                    marginBottom: '10px',
                  }}>Detail</Button>
...
export default function Home() {
    ...
    const getData = async () => {
        const result = await fetch('http://192.168.0.35/feeder-backend/public/api/kelas/history?page=' + (page+1),{
            method: 'POST',
            headers: {
