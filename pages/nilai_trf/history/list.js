...
const columns = [
    ...
    { field: 'trf_head_catatan', headerName: 'Catatan', width: 130 },
    { field: 'trf_head_total', headerName: 'Jumlah Record', width: 130 },
    { field: 'user_fullname', headerName: 'Oleh', width: 150, renderCell: (params) => {
        return (
            <>{params.row.user_fullname} <br/>pada : {params.row.trf_head_created_at}</>
        );
    }},
    {
        field: 'status_error', headerName: 'Action', width: 150, renderCell: (params) => {
            return (
                <Link href={"/nilai_trf/history/"+params.row.trf_head_id}>
                  <Button style={{
                    marginTop: '10px',
                    marginBottom: '10px',
                  }}>Detail</Button>
...
export default function Home() {
    ...
    const getData = async () => {
        const result = await fetch('http://192.168.0.35/feeder-backend/public/api/nilaiTransfer/history?page=' + (page+1),{
            method: 'POST',
            headers: {
                ...
                <Card variant="outlined">
                    <CardContent>
                        <Typography variant="h5" gutterBottom align='left'>
                            List History Import Nilai Transfer
                        </Typography>
                        <div style={{ height: '100vh', width: '100%', marginTop: '15px' }}>
                            <DataGrid
                                getRowId={(row) => row.trf_head_id}
                                rows={data}
                                columns={columns}
                                pageSize={100}
